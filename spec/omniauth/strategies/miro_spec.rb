...
describe OmniAuth::Strategies::Miro do
  ...
  subject do
    ...
    OmniAuth::Strategies::Miro.new(*args).tap do |strategy|
  ...
  describe 'client options' do
    it 'should have correct name' do
      expect(subject.options.name).to eq('miro')
    end

    it 'should have correct site' do
      expect(subject.options.client_options.site).to eq('https://api.miro.com')
    end
  ...
  describe 'info' do
    ...
    it 'should returns the urls' do
      ...
      expect(subject.info[:urls]['Miro']).to eq("https://miro.com/#{raw_info_hash['screen_name']}")
    end
  end
  ...
  describe 'request_phase' do
    ...
    context 'with a specified callback_url in the params' do
      before do
        params = { 'callback_url' => 'http://foo.dev/auth/miro/foobar' }
        allow(subject).to receive(:request) do
      ...
      it 'should use the callback_url' do
        expect(subject.callback_url).to eq 'http://foo.dev/auth/miro/foobar'
      end

      it 'should return the correct callback_path' do
        expect(subject.callback_path).to eq '/auth/miro/foobar'
      end
    end
    ...
    context 'with no callback_url set' do
      ...
      it 'should return the default callback_path value' do
        expect(subject.callback_path).to eq '/auth/miro/callback'
      end
    end
